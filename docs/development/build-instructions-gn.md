...
| Linux x64   | Windows x64   | Experimental         |
...
#### Windows on Linux (experimental)

To cross-compile for Windows on Linux, [follow Chromium's cross-compilation guide](https://chromium.googlesource.com/chromium/src/+/master/docs/win_cross.md) to setup the necessary environment variables. To do this, you'll need to have the necessary dependencies - [Windows requirements for building Chromium](https://chromium.googlesource.com/chromium/src/+/refs/heads/main/docs/windows_build_instructions.md#Visual-Studio) - installed on a Windows machine. Follow the steps to generate the ```<hash_value>.zip``` file on Windows, and move that to your Linux build machine. Then set the two environment variables ```DEPOT_TOOLS_WIN_TOOLCHAIN_BASE_URL``` and ```GYP_MSVS_HASH_<toolchain hash>``` as specified in the Chromium cross-compilation document.

To run this, you also need to have ```libfuse2``` installed.

Next, in your .gclient file, add the ```target_os=['win']``` line (or append 'win' to the list if it already exists)
```
solutions = [
  {
    ...
...
##### Using Electron Build Tools

The build arguments for electron are found in the ```//electron/build/args/<config_name>.gn``` file. (// refers to the ```src``` directory generated by ```e init```, <config_name>.gn can be release.gn or testing.gn or your custom config name depending on the active build config - to find the active build config, run ```e show current```. To find the args file in use, you can also read the first line when running ```e build```).
Since we cannot directly inject ```target_os="win"``` as a GN argument when using electron build tools, we append this to the arg files that are used by our build. To do this, edit the <config_name>.gn file to include the following line:
```
target_os="win"
```

Then run ```e build``` as above to run the build process, which will run ```gn gen ...``` internally and generate the windows build in the appropriate output directory.

##### Using GN natively

Run `gn gen` as above with `target_os="win"` as an additional argument.

##### Known Problems

**fatal error**: cannot open file, too many files open

This error pertains to the limits that Linux places on number of open file handles. You can see the limits by running ```ulimit -n```. To modify these limits temporarily, you can run ```ulimit -n <desired limit>```. For permanently changing limits, you can find guides online depending on whether or not you use systemd.
